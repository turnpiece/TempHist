 ...
 */
async function getCityFromCoords(lat: number, lon: number): Promise<{ location: string; countryCode: string | null }> {
  try {
    ...
    if (city && !country && data.display_name) {
      ...
      if (lastPart && lastPart !== city) {
        return { location: `${city}, ${lastPart}`, countryCode };
      }
    }
    ...
    if (data.display_name) {
      const displayParts = data.display_name.split(',').map(part => part.trim());
      const location = displayParts.length >= 2
        ? `${displayParts[0]}, ${displayParts[displayParts.length - 1]}`
        : displayParts[0];
      return { location, countryCode };
    }

    // Ultimate fallback
    return { location: DEFAULT_LOCATION, countryCode: null };
  } catch (error) {
 ...
 */
export function detectUserLocationWithGeolocation(): Promise<{ location: string; countryCode: string | null; latitude: number; longitude: number }> {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position: GeolocationPosition) => {
        try {
          ...
          const { location, countryCode } = await getCityFromCoords(latitude, longitude);
          resolve({ location, countryCode, latitude, longitude });
        } catch (error) {
          reject(error);
        }
      },
 ...
 */
export async function getLocationFromIP(): Promise<{ location: string; countryCode: string | null; timezone: string | null; latitude: number | null; longitude: number | null } | null> {
  try {
