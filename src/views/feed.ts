...
const FILTER_OPTIONS: Array<{ label: string; period: Period | '' }> = [
  ...
  { label: 'Past Week', period: 'weekly' },
  { label: 'Past Month', period: 'monthly' },
  { label: 'Past Year', period: 'yearly' },
];

const TOOLTIP_TEXT =
  'About this data: This feed shows charts manually generated by our visitors. ' +
...
export function renderFeedPage(): void {
  ...
  tooltipWrap.setAttribute('role', 'tooltip');
  tooltipWrap.setAttribute('aria-label', TOOLTIP_TEXT);
  tooltipWrap.setAttribute('tabindex', '0');

  const tooltipIcon = document.createElement('span');
  tooltipIcon.className = 'feed-tooltip__icon';
  tooltipIcon.setAttribute('aria-hidden', 'true');
  tooltipIcon.textContent = 'ℹ';

  const tooltipBubble = document.createElement('span');
  tooltipBubble.className = 'feed-tooltip__bubble';
  tooltipBubble.textContent = TOOLTIP_TEXT;

  tooltipWrap.appendChild(tooltipIcon);
  tooltipWrap.appendChild(tooltipBubble);

  header.appendChild(h2);
  header.appendChild(tooltipWrap);

  const desc = document.createElement('p');
  desc.className = 'feed-desc';
  desc.textContent =
    'See what TempHist users are discovering. Below are recent temperature history charts generated by people around the world. Upward trends highlighted in red indicate long-term warming.';
