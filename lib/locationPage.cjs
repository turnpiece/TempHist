...
/**
 * "America/New_York" → "America/New York", for display only.
 *
 * IANA identifiers use underscores where the place name has a space. Never apply
 * this to a value that is passed back to code — the bootstrap payload keeps the
 * real identifier, because Intl and the API both need the canonical form.
 */
  ...
  return `<section id="locationIntro" class="location-intro" data-slug="${escapeAttr(loc.slug)}">
      ...
      <div class="location-intro__body${media ? '' : ' location-intro__body--no-media'}">
        ...
        <div class="location-intro__text">
          <p class="location-intro__lede">${escapeHtml(region)} · ${escapeHtml(coords)} · ${escapeHtml(formatTimezone(loc.timezone))}</p>
          ...
          <p>Switch between daily, weekly, monthly and yearly views to compare a single
            ...
            (${escapeHtml(formatTimezone(loc.timezone))}), not yours, so "today" means today in
